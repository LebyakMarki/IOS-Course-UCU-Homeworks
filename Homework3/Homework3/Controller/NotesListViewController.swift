    ...
    override func viewDidLoad() {
        ...
        func searchByNameHandler(action: UIAction) {
            ...
            alert.addAction(UIAlertAction(title: "Search", style: .default, handler: { [weak alert] (_) in
                    let textField = alert?.textFields![0]
                    if textField?.text != "" {
                        let foundNote: NoteActive? = ViewController.notesManager.searchNoteByName(noteName: (textField?.text)!)
                        if foundNote != nil {
        ...
        func searchByWordHandler(action: UIAction) {
            ...
            alert.addAction(UIAlertAction(title: "Search", style: .default, handler: { [weak alert] (_) in
                    let textField = alert?.textFields![0]
                    if textField?.text != "" {
                        let foundNote: NoteActive? = ViewController.notesManager.searchNoteByWord(noteWord: (textField?.text)!)
                        if foundNote != nil {
                            self.performSegue(withIdentifier: "editNoteSegue", sender: foundNote)
                        }
                    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "editNoteSegue" {
            ...
            if let selectedCell = sender as? IndexPath {
                ...
                case .deleted:
                    destinationVC.noteDeleted = ViewController.notesManager.notesDeletedArray[selectedCell.row]
